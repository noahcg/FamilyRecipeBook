...
export function BillingCard({ billing }: { billing: BillingStatus }) {
  ...
  return <section className="scroll-mt-6 border-b border-line-soft pb-8">
    ...
    <div className="recipe-card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
      <div><p className="font-bold text-green-deep">{isPlus ? "Home Cooked Plus" : "Home Cooked Free"}</p><p className="mt-1 text-sm text-ink-muted">{billing.tier === "grandfathered" ? "Lifetime Plus access granted by Home Cooked." : isPlus ? `Status: ${billing.status}${billing.current_period_end ? ` · renews ${new Date(billing.current_period_end).toLocaleDateString()}` : ""}${billing.cancel_at_period_end ? " · cancellation scheduled" : ""}` : "One cookbook, 50 saved recipes, and 5 AI ideas per month."}</p>{!isPlus && <PlusPlanDialog />}</div>
      {billing.tier === "grandfathered" ? null : isPlus ? <BillingButton mode="portal">Manage Billing</BillingButton> : <BillingButton>Upgrade to Plus</BillingButton>}
    </div>
  </section>;
}
